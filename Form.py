...
def auto_save(name, position, room_type, address, relationships, reason, marital_status, children_count, id_card, house_rights, phone_number):
    ...
    with open("form_data.csv", mode="a", newline="", encoding="utf-8") as file:
        ...
        if file.tell() == 0:  # ตรวจสอบว่าไฟล์ว่างหรือไม่
            writer.writerow([
                "ชื่อ-นามสกุล", "ตำแหน่ง", "ประเภทที่พักอาศัย", "ที่อยู่",
                "ความสัมพันธ์ในบ้าน", "เหตุผลการยื่นคำร้อง", "สถานภาพ", "จำนวนบุตร", "หมายเลขบัตรประชาชน",
                "หมายเลขโทรศัพท์ติดต่อ", "สิทธิในการเบิกค่าบ้าน", "วันที่บันทึก"
            ])
        writer.writerow([name, position, room_type, address, relationships, reason, marital_status, children_count, id_card, phone_number, house_rights, datetime.now().strftime("%Y-%m-%d %H:%M:%S")])
...
# ข้อมูลสถานภาพสมรส
st.subheader("ข้อมูลสถานภาพสมรส")
...
if st.button("บันทึกข้อมูล", use_container_width=True):
    auto_save(name, position, room_type, address, ", ".join(relationships), reason, marital_status, children_count, id_card, house_rights, phone_number)
