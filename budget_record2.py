...
if page == "บันทึกข้อมูล":
    st.title("บันทึกข้อมูลการใช้จ่าย")
    st.write(df)
    st.write("กรอกจำนวนเงินที่ใช้ไปแล้วในแต่ละรายการ")

    # ฟอร์มกรอกข้อมูลการใช้จ่าย
    for index, row in df.iterrows():
        amount_spent = st.number_input(f"จำนวนเงินที่ใช้ไปแล้วในรายการ {row['รายการงบประมาณ']}", min_value=0, value=row['จำนวนเงินที่ใช้ไปแล้ว (บาท)'])
        df.at[index, 'จำนวนเงินที่ใช้ไปแล้ว (บาท)'] = amount_spent
